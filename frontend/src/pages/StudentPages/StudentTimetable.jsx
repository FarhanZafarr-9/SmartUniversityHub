...
import { getStudentCourses, getTimetableByCourse } from '../../../services/servicesApi';
...
const StudentTimetable = () => {
    ...
    const studentId = JSON.parse(localStorage.getItem('currentUser'))?.user?.user_id;

    // Fetch enrolled courses and timetable
    const fetchTimetableData = async () => {
        try {
            setLoading(true);

            // Fetch enrolled courses
            const enrolledCourses = await getStudentCourses(studentId, 'enrolled');
            console.log('Enrolled Courses:', enrolledCourses);
            setCourses(enrolledCourses);

            // Fetch timetable for each course
            const timetablePromises = enrolledCourses.map((course) =>
    ...
    return (
        <div className="h-full w-full p-6 overflow-y-auto bg-dark-semi-dark rounded-xl border border-[#55555555]">
            ...
            <div className="mb-6 bg-dark-glassy rounded-xl p-4 border border-[#55555555]">
                <h1 className="text-xl font-semibold text-gray-200">Student Timetable</h1>
